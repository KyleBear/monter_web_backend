...
class GoogleSheetsRankUpdater:
    """Google Sheets 순위 업데이트 클래스 (비동기 처리)"""
    ...
    async def get_rank_by_naver_api_async(
        self,
        session: aiohttp.ClientSession,
        keyword: str,
        nvmid: str,
        price_nvmid: Optional[str] = None,
        max_rank: int = 1000
    ) -> Union[int, str, None]:
        """
        네이버 OpenAPI로 순위 조회 (비동기)
        ...
        try:
            ...
            logger.debug(f"OpenAPI 순위 조회 (비동기): keyword='{keyword}', nvmid='{target_nvmid}'")
            ...
            for page in range(1, max_pages + 1):
                ...
                async with self.semaphore:  # 동시 호출 수 제한 (워커 수 10개)
                    try:
                        # 비동기로 네이버 API 호출
                        api_results = await self._call_naver_api_async(
                            session,
                            keyword,
                            display=100,
                            start=start,
    ...
    async def _call_naver_api_async(
        self,
        session: aiohttp.ClientSession,
        keyword: str,
        display: int = 100,
        ...
        try:
            async with session.get(API_URL, headers=headers, params=params, timeout=aiohttp.ClientTimeout(total=10)) as response:
                ...
                logger.debug(f"네이버 오픈 API 응답 (비동기): 총 {total}개 결과, {len(items)}개 반환, sort={sort}, start={start}")
            ...
            logger.error(f"네이버 오픈 API 요청 실패 (비동기): {e}", exc_info=True)
            raise
        except Exception as e:
            logger.error(f"네이버 오픈 API 호출 중 예상치 못한 오류 (비동기): {e}", exc_info=True)
            raise
    ...
    async def process_batch_async(
        ...
        for data in batch_data:
            ...
            price_nvmid = data.get('price_nvmid')
            
            if keyword and nvmid:
                task = self.get_rank_by_naver_api_async(
                    session,
                    keyword,
                    nvmid,
                    price_nvmid
        ...
        for result in results:
            if isinstance(result, Exception):
                logger.error(f"비동기 처리 중 오류: {result}", exc_info=True)
    ...
    def update_all_ranks(self, insert_column_first: bool = True) -> Dict:
        ...
        try:
            logger.info("=" * 60)
            logger.info("Google Sheets 순위 업데이트 시작 (비동기 처리)")
            ...
            # 2. 비동기로 순위 조회 및 업데이트 (200개씩 배치 처리)
            logger.info("\n[3단계] 순위 조회 및 업데이트 시작 (비동기, 200개씩 배치 처리)...")
            ...
            async def process_all_batches():
                nonlocal total_success_count, total_unavailable_count, total_empty_count
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    for batch_start in range(0, total_count, batch_size):
                        ...
                        # 비동기로 배치 처리
                        batch_ranks = await self.process_batch_async(session, batch_data)
                        
                        # 통계 계산
                        batch_success_count = sum(1 for r in batch_ranks if r and isinstance(r, int))
                        batch_unavailable_count = sum(1 for r in batch_ranks if r == "확인불가")
                        batch_empty_count = sum(1 for r in batch_ranks if r is None)
